...
export function FaqTurnaroundCalculator() {
  ...
  return (
    <div className="bg-white dark:bg-slate-900 border border-[var(--border)] rounded-3xl p-6 sm:p-10 shadow-xl relative overflow-hidden my-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-7 space-y-6">
          {/* Project Type */}
          <div>
            <label className="block text-xs font-mono uppercase text-[var(--text-muted)] font-bold mb-2.5">
            ...
            <div className="grid grid-cols-3 gap-2.5">
              {[
                ...
                >
                  ...
                  <div className={`text-[10px] ${sizeTier === s.id ? 'text-slate-900' : 'text-[var(--text-muted)]'}`}>
                    {s.sub}
                  </div>
                </button>
              ))}
                  ...
                  <button
                    ...
                    className={`flex items-center gap-2.5 p-3 rounded-xl border text-left text-xs font-medium transition-all cursor-pointer ${
                      isSelected
                        ...
                        : 'bg-[var(--surface)] border-[var(--border)] text-[var(--text-secondary)] hover:border-amber-400/50'
                    }`}
                  >
                    <div
                      className={`w-4 h-4 rounded flex items-center justify-center text-[10px] ${
                        isSelected ? 'bg-amber-500 text-slate-950' : 'border border-[var(--border)] bg-white'
                      }`}
                    >
        ...
        <div className="lg:col-span-5 bg-[var(--surface)] border border-[var(--border)] rounded-2xl p-6 sm:p-7 flex flex-col justify-between space-y-6">
          <div>
            <span className="text-[11px] font-mono uppercase tracking-wider text-[var(--text-muted)] font-bold block mb-1">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-2xl font-black text-[var(--foreground)] font-heading">
                  {getTurnaround()}
                </div>
            ...
            <div className="my-5 border-t border-[var(--border)] pt-5 space-y-3">
              <div className="flex justify-between items-center text-xs">
                <span className="text-[var(--text-secondary)] font-medium">Estimated Drawing Sheets:</span>
                <span className="font-mono font-bold text-[var(--foreground)]">{getSheetCount()} full-size sheets</span>
              </div>
              <div className="flex justify-between items-center text-xs">
                <span className="text-[var(--text-secondary)] font-medium">PE Stamp Eligibility:</span>
                <span className="font-bold text-amber-600 dark:text-amber-400">All 11 Licensed States</span>
              </div>
              <div className="flex justify-between items-center text-xs">
                <span className="text-[var(--text-secondary)] font-medium">Plan Check Guarantee:</span>
                <span className="font-bold text-emerald-600 dark:text-emerald-400">100% Free Corrections</span>
              </div>
            </div>
          </div>
